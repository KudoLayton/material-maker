...
func inspect_example(data: Dictionary, name: String) -> void:
	for node in data.get("nodes", []):
		if node.get("type") == "particle_node":
			...
			check(not (settings.kind in ["constant", "uniform"] and settings.get("data_type") in ["float", "vec3", "vec4"]), name + ": ordinary values and parameters use stock nodes")
...
func run():
	...
	for name in ["blank", "gravity", "collision", "subparticle", "library_module", "library_gravity"]:
		...
		if name in ["gravity", "collision", "library_gravity"]:
			check(data.nodes.any(func(n): return n.get("type") == "remote"), name + ": Remote exposes ordinary effect parameters")
		...
		if name == "gravity":
			...
			var remote = graph.get_node("EffectParameters")
			remote.set_parameter("launch_speed", 8.0)
			remote.set_parameter("gravity_y", -4.0)
			await probe(graph, "abs(VELOCITY.y - (8.0 - 4.0 * DELTA)) < 0.0001", "remote_gravity", "TRANSFORM = mat4(1.0); VELOCITY = vec3(0.0, 8.0, 0.0);")
		graph.queue_free()
