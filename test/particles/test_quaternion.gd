...
func run():
	...
	for values in [[Quaternion.IDENTITY, Quaternion.IDENTITY], [Quaternion(Vector3.UP, 0.8), Quaternion(Vector3.RIGHT, 0.6)], [Quaternion(Vector3.RIGHT, 0.6), Quaternion(Vector3.UP, 0.8)], [Quaternion(0,0,0,0), Quaternion.IDENTITY], [Quaternion(0,0,0,-2), Quaternion(Vector3.UP, 0.8)]]:
		...
		graph.queue_free()
		restored.queue_free()
	var ordinary = await mm_loader.create_gen({"nodes": [], "connections": []})
	add_child(ordinary)
	var euler = await add(ordinary, definitions["Euler to Quaternion"].duplicate(true))
	var angles = await add(ordinary, {"type": "shader", "name": "Angles", "shader_model": {"name": "Angles", "parameters": [], "inputs": [], "outputs": [{"type": "rgb", "rgb": "vec3(0.0, 0.0, 90.0)"}]}})
	var product = await add(ordinary, definitions["Quaternion Multiply"].duplicate(true))
	ordinary.connect_children(angles, 0, euler, 0)
	ordinary.connect_children(euler, 0, product, 0)
	var rendered: Image = await product.render_output(0, Vector2i(8, 8))
	var pixel := rendered.get_pixel(4, 4)
	check(abs(pixel.b - sqrt(0.5)) < 0.001 and abs(pixel.a - sqrt(0.5)) < 0.001, "Ordinary MM compute renderer supports shared quaternion functions")
	var sink = await add(ordinary, {"type": "shader", "shader_model": {"name": "Sink", "parameters": [], "inputs": [{"name": "q", "type": "rgba", "default": "vec4(0.0)"}], "outputs": [{"type": "rgba", "rgba": "$q($uv)"}]}})
	ordinary.connect_children(product, 0, sink, 0)
	var module = ordinary.create_subgraph([angles, euler, product])
	var lib = load("res://material_maker/tools/library_manager/library.gd").new()
	lib.create_library("res://app_quaternion_library.json", "Quaternion module test")
	lib.add_item("Modules/Quaternion", null, module.serialize())
	lib.load_library("res://app_quaternion_library.json")
	var reused = await mm_loader.create_gen(lib.get_item("Modules/Quaternion").item)
	ordinary.add_generator(reused)
	ordinary.connect_children(reused, 0, sink, 0)
	rendered = await sink.render_output(0, Vector2i(8, 8))
	pixel = rendered.get_pixel(4, 4)
	check(abs(pixel.b - sqrt(0.5)) < 0.001 and abs(pixel.a - sqrt(0.5)) < 0.001, "Quaternion library module output roundtrip")
	lib.free()
	ordinary.queue_free()
