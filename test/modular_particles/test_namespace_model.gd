...
func run() -> void:
	...
	var duplicate_doc: Dictionary = doc.duplicate(true)
	duplicate_doc.modules[module_id].inputs.append({"id":"velocity_variant","name":"Position","type":"vec3","default":[0.0,0.0,0.0]})
	var duplicate_ctx := Presentation.context(duplicate_doc,module_id)
	var duplicate_a := Presentation.describe("module_parameter","velocity","","",duplicate_ctx)
	var duplicate_b := Presentation.describe("module_parameter","velocity_variant","","",duplicate_ctx)
	check(not duplicate_a.badge.is_empty() and duplicate_a.display != duplicate_b.display,"same-name inputs are disambiguated within their module scope")
