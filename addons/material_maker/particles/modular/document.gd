...
static func create() -> Dictionary:
	return {"type":"mm_particle_effect", "version":2, "target":"4.7.2", "user_parameters":[], "attributes":[], "modules":{}, "stages":{"spawn":[], "update":[]}, "emitter":{"rate":64.0,"duration":1.0,"loop":true,"bursts":[],"lifetime":1.0}, "renderer":{"mode":"additive","billboard":true,"quad_size":0.1,"custom_attribute":"custom"}}

static func shape_error(data: Dictionary) -> String:
	if data.get("type") != "mm_particle_effect" or data.get("version") != 2 or data.get("target") != "4.7.2": return "Unsupported document format or target: expected .mpfx version 2 for Godot 4.7.2 (no automatic migration)"
