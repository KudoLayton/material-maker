...
파티클 문서를 열면 기존 **3D Preview**에서 현재 그래프를 실행합니다. 카메라 회전과 확대·축소는 기존 조작을 사용하며, 일반 재질 문서로 전환하면 메시 미리보기로 돌아갑니다. 그래프 배경의 3D Preview도 지원합니다.

미리보기 메뉴의 **Particles**에서 다음 항목을 설정합니다.

| 항목 | 동작 | 기본값 |
|---|---|---|
| Emission | Continuous: 계속 나누어 방출 / Burst: 동시에 방출하고 수명 주기로 자동 반복 | Continuous |
| Shape | Square: 흰색 사각형 / Soft Circle: 중앙의 흰색이 가장자리에서 투명해지는 원형 | Soft Circle |
| Amount | 파티클 개수, 정수 1–100000 | 128 |
| Lifetime (s) | 파티클 시스템의 수명 및 셰이더 `LIFETIME`, 0.01–3600초 | 2초 |
| Quad Size | Quad의 기본 가로·세로 크기, 0.001–1000 | 0.1 |
| Pause / Restart | 일시정지·재개 / 시뮬레이션 초기화 후 재생 | 재생 |

두 외형은 조명 영향을 받지 않는 카메라 방향의 Quad이며, 그래프의 `COLOR`와 Transform 크기를 반영합니다. 빌보드 재질이 방향을 정하므로 Transform의 회전 자체를 확인하는 용도는 아닙니다. `CUSTOM`을 회전·애니메이션 채널로 예약하지 않습니다.

설정은 `.ptex`의 미리보기 전용 데이터로 저장되며 셰이더 uniform이나 export 결과에 포함하지 않습니다. 이전 파일은 기본값으로 열립니다. 방출 설정·셰이더·파라미터·텍스처가 바뀌면 재시작하고, 노드 이동이나 카메라 조작만으로는 재시작하지 않습니다. 비교를 위해 시스템 난수 시드는 0으로 고정하며, 숨겨진 미리보기는 정지합니다. 재시작 시 GPU 버퍼를 준비한 다음 Quad를 표시합니다.

Typed Parameter 기본값과 배열, 기존 라이브러리·Remote·서브그래프·Image·Buffer를 미리보기에서도 사용합니다. 외부 sampler 리소스는 지정한 Godot 프로젝트에서 읽습니다. 다른 프로젝트의 import나 간접 참조 때문에 읽을 수 없는 리소스는 오류로 표시합니다. 잘못된 그래프나 리소스는 마지막 정상 화면을 정지하고 오류를 표시하며, 수정하면 복구합니다.

미리보기는 원점의 단일 이미터입니다. 별도 충돌체·Attractor·Sub Emitter·사용자 지정 Draw Pass를 배치하는 기능은 제공하지 않습니다. 실제 게임의 개수·수명·Draw Pass·충돌체·Sub Emitter 연결은 Godot에서 설정합니다. 기존 2D 이미지 미리보기는 상태에 의존하지 않는 노드에 사용하며, 파티클 상태를 이미지로 평가하지 않습니다.
